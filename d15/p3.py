...
if __name__ == "__main__":
    ...
    for tunel in tunels:
        direction = update_direction(direction, tunel[0])
        new_point = point + direction * int(tunel[1:])
        new_discrete_point = np.array(
            (
                np.searchsorted(discrete_points_0, new_point[0]),
                np.searchsorted(discrete_points_1, new_point[1]),
            )
        )

        for el_0 in range(
            min(discrete_point[0], new_discrete_point[0]),
            max(discrete_point[0], new_discrete_point[0]) + 1,
        ):
            for el_1 in range(
                min(discrete_point[1], new_discrete_point[1]),
                max(discrete_point[1], new_discrete_point[1]) + 1,
            ):
                world.add(
                    (
                        int(discrete_points_0[el_0]),
                        int(discrete_points_1[el_1]),
                    )
                )

        point = new_point
        discrete_point = new_discrete_point
